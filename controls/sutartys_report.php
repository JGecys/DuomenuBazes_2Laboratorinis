...
<div id="content">
	<div id="contentMain">
			...
			<div id="formContainer">
				...
				<form action="" method="post">
					<fieldset>
						...
						<p><label class="field" for="pavarde">Užsakovo pavardė (neprivaloma)</label><input type="text" id="pavarde" name="pavarde" class="textbox-100" value="<?php echo isset($fields['pavarde']) ? $fields['pavarde'] : ''; ?>" /></p>
					</fieldset>
		...
		<?php } else {
			...
			$contractData = $contractsObj->getContracts($data['dataNuo'], $data['dataIki'], $data['pavarde']);
			$totalPrice = $contractsObj->getSumPriceOfContracts($data['dataNuo'], $data['dataIki'], $data['pavarde']);
			$totalServicePrice = $contractsObj->getSumPriceOfOrderedServices($data['dataNuo'], $data['dataIki'], $data['pavarde']);
			...
			if(sizeof($contractData) > 0) { ?>
				...
				<table class="reportTable">
					...
					for($i = 0; $i < sizeof($contractData); $i++) {
						...
						if($contractData[$i]['sutarties_papildomos_irangos_kaina'] == 0) {
							$contractData[$i]['sutarties_papildomos_irangos_kaina'] = "neužsakyta";
						} else {
							$contractData[$i]['sutarties_papildomos_irangos_kaina'] .= " &euro;";
						}
						...
						echo
							...
							. "<td>{$contractData[$i]['sutarties_papildomos_irangos_kaina']}</td>"
							. "</tr>";
						if($i == (sizeof($contractData) - 1) || $contractData[$i]['asmens_kodas'] != $contractData[$i+1]['asmens_kodas']) {

							if($contractData[$i]['bendra_kliento_irangos_kaina'] == 0) {
								$contractData[$i]['bendra_kliento_irangos_kaina'] = "neužsakyta";
							} else {
								$contractData[$i]['bendra_kliento_irangos_kaina'] .= " &euro;";
							}
							...
							echo
								...
								. "<td class='border'>{$contractData[$i]['bendra_kliento_irangos_kaina']}</td>"
								. "</tr>";
						}
					}
					?>

...
if(!empty($_POST['submit'])) {
    ...
    if($validator->validate($_POST)) {
        ...
        if(isset($data['editing'])) {
            // redaguojame klientą
            $employeesObj->update($data);
        } else {
            // įrašome naują klientą
            $employeesObj->insert($data);
        }
...
<div id="formContainer">
    ...
    <form action="" method="post">
        <fieldset>
            ...
            <p>
                <label class="field"
                       for="name">Padalinys<?php echo in_array('fk_padalinys', $required) ? '<span> *</span>' : ''; ?></label>
                <select name="fk_padalinys" title=''>
                    <?php
                    ...
                    foreach ($padaliniaiList as $key => $val) {
                        $selected = '';
                        if(isset($fields['fk_padalinys']) && $fields['fk_padalinys'] == $val['id']){
                            $selected = 'selected';
                        }
